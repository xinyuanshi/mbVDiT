...
parser.add_argument("--sc_data", type=str, default='_SA_1695.h5ad')
parser.add_argument("--st_data", type=str, default='_SA_1695.h5ad')
parser.add_argument("--document", type=str, default='data_sa')
parser.add_argument("--device", type=str, default='cuda:0')
parser.add_argument("--batch_size", type=int, default=256)  # 256
parser.add_argument("--hidden_size", type=int, default=64)  # 512
parser.add_argument("--diffusion_step", type=int, default=200) #200
parser.add_argument("--learning_rate", type=float, default=1e-4)
parser.add_argument("--epoch", type=int, default=300) #300
parser.add_argument("--depth", type=int, default=4) # 16
parser.add_argument("--noise_std", type=float, default=10)
parser.add_argument("--head", type=int, default=4) # 16
parser.add_argument("--mask_nonzero_ratio", type=float, default=0.5)
...
def train_valid_test():
    ...
    text_path = "D:/sa_sample_type.csv"
    ...
    age = pd.read_csv("D:/sa_sample_age.csv", header=0, index_col=False)
    max_age = int(age.values.max())

    model = DiT_diff(
        st_input_size=128,
        condi_input_size=128,
    ...
    with torch.no_grad():
       test_gt = torch.stack([data for data, _, _, _, _, _, _ in train_dataset])
       org_data_array = torch.stack([data for _, _, _, _, _, _, data in train_dataset])
       nonzero_mask = torch.stack([data for _, _, _, _, data, _, _ in train_dataset])
       ...
       prediction = sample_diff(model,
                                ...
                                sample_shape=(test_gt.shape[0], 128),
                                is_condi=True,
                                sample_intermediate=diffusion_step,
                                model_pred_type='noise',
                                is_classifier_guidance=False,
