...
def model_sample_diff(model, device, dataloader, total_sample, time, is_condi, ob_cond, condi_flag, vae_model):
    noise = []
    i = 0
    for x, x_cond, cond, age_cond, _, _, _ in dataloader: # 计算整个shape得噪声 一次循环算batch大小  加上了celltype 去掉了, celltype
        ...
        if not is_condi:
            ...
            n = model(total_sample[i:i+len(x)], t, cond_emb, age_cond, ob=torch.tensor(latent_data).float().to(device), condi_flag=condi_flag) # 加上了celltype 去掉了, celltype
        noise.append(n)
        i = i+len(x)
...
def sample_diff(model,
    ...
    vae_model.load_state_dict(torch.load('D:/sa_vae_parameters_tune.pth'))
